...
//---------------------------------------------------------------------------
void __fastcall TForm1::durationChange(TObject *Sender)
{
    char buffer[64];
	dur=atoi(duration->Text.c_str());
    sprintf(buffer,"%d",dur);
    duration->Text=buffer;
}
//---------------------------------------------------------------------------
void __fastcall TForm1::vrememejdustimuliteChange(TObject *Sender)
{
    char buffer[64];
	vrmejdustimuli=atoi(vrememejdustimulite->Text.c_str());
    sprintf(buffer,"%d",vrmejdustimuli);
    vrememejdustimulite->Text=buffer;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
    ...
    fprintf(fp,"\nПродължителност:\n");
    fprintf(fp,duration->Text.c_str());
    fprintf(fp,"\nВреме между стимули:\n");
    fprintf(fp,vrememejdustimulite->Text.c_str());
    fprintf(fp,"\nБрой на стимулите:\n");
...
//---------------------------------------------------------------------------
  ...
  cbboxFlush();
  cbboxBuzzer(respSEC02,respTONE5);//звук
//---------------------------------------------------------------------------
  vsgSetDisplayPage(0);
  vsgSetDrawPage(vsgVIDEOPAGE,0,0);
 //Стимули
 for(int i=1;i<=n;i++){
 //Време между стимулите
 for(int j=0;j<=vrmejdustimuli;j++)
 vsgFrameSync();
//---------------------------------------------------------------------------
  cbboxOpen(respCOM1);//Initiates communications to the response box
  cbboxFlush();
...
//---------------------------------------------------------------------------
        if(s==1){
           sluchainovreme==time1;
        }
//---------------------------------------------------------------------------
        if(s==2){
           sluchainovreme==time2;
        }
//---------------------------------------------------------------------------
        if(s==3){
           sluchainovreme==time3;
        }
//---------------------------------------------------------------------------
        if(s==4){
           sluchainovreme==time4;
        }
//---------------------------------------------------------------------------
        if(s==5){
           sluchainovreme==time5;
        }
//---------------------------------------------------------------------------
        if(s==6){
           sluchainovreme==time6;
        }
//---------------------------------------------------------------------------
        if(s==7){
           sluchainovreme==time7;
        }
     }
 //---------------------------------------------------------------------------
 for(int k=0;k<=dur;k++){
 ...
 //---------------------------------------------------------------------------
 for(int proverkanasluchainovreme=0;proverkanasluchainovreme<=sluchainovreme;proverkanasluchainovreme++)
 cbboxBuzzer(respSEC02,respTONE0);
 for(int vremezaotgovor=1;vremezaotgovor<=301;vremezaotgovor++){
     ...
     if(cbboxCheck((CBBOXRECORD*)&CBBox)==respEMPTY){
        if(vremezaotgovor==301){
           if(flag){
              fprintf(fp,"?? %d стимул не е посочен отговор.",i);
              f=0;
              i--;
